...
class FairyBuffOnStart(OnStart):
    ...
    def handle(self, stack, *args, **kwargs):
        highest_attack_evil = max([char.attack for char in self.source.player.valid_characters(_lambda=lambda char: Tribe.EVIL in char.tribes)] + [0])
        highest_health_good = max([char.health for char in self.source.player.valid_characters() if Tribe.GOOD in char.tribes] + [0])

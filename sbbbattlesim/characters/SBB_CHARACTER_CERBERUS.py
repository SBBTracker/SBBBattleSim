...
class GrimSoulOnDeath(OnDeath):
    ...
    def handle(self, stack, *args, **kwargs):
        ...
        if targets:
            ...
            for _ in range(itr):
                with stack.open(killed_character=None, reason=ActionReason.GRIMSOUL_SLAY_TRIGGER) as executor:  # does golden grim soul open two stacks or just the one. If just the one, flip this w/ the above for loop
                    for slay in slays:

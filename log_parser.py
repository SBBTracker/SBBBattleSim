    ...
    def __init__(self, info, game_state=GameState.UNKNOWN):
        ...
        if info is not None:
            ...
            if self.action_type == EVENT_ADDPLAYER or self.action_type == EVENT_ENTERRESULTSPHASE:
                ...
                self.attrs = ['cardname', 'cardattack', 'cardhealth', 'is_golden', 'slot', 'zone', 'cost', 'level',
                              'content_id']
...
def run(queue: Queue, log=logfile):
    ...
    while True:
        ...
        for action in parse(ifs):
            ...
            elif not inbrawl and not current_player_stats and action.task == TASK_ADDPLAYER and prev_action is not None and prev_action.task == TASK_GETTHISPLAYER:
